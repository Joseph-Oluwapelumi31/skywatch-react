...
export default function CurrentWeather({weatherData, isLoading}) {
    ...
    const getHourly = (date)=>{
        ...
        .map((t, i)=> ({time:t , temp: weatherData.hourly.temperature[i] }))
    ...
    return(
            ...
            <div>
                ...
                <div className='relative bg-[var(--neutral-700)] p-4 text-sm rounded-xl gap-2 flex flex-col sm:h-full md:h-full '>
                    ...
                    {toggle && 
                    <div className="absolute right-4 top-15 flex flex-col gap-2 bg-[var(--neutral-800)]  w-[40%] cursor-pointer p-4 rounded-2xl"> 
                        {Array.from({ length: 7 }).map((_, i) => {
                          ...
                          return <p onClick={()=>{setDate(formattedDate)}} key={i}>{weekday}</p>;
                        })}
                    ...
                    <div className='flex justify-between items-center '>
                         ...
                         <button onClick={handleToggle} className='flex py-2 text-xs px-4 bg-[var(--neutral-600)] rounded-xl gap-2 items-center'>
                     ...
                     {getHourly(date).map((hourly, index)=>
                        (
                        <div key={index} className='bg-[var(--neutral-600)] flex  justify-between items-center p-2 rounded-xl'>
                             <div className='flex flex-row gap-2 items-center'>
                                 <img src="assets/images/icon-partly-cloudy.webp" alt="partly cloudy" width={30} height={30} />
